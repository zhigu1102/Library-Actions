...
#     "Referer": "https://lxl.sdyu.edu.cn/web/seat3?area=20&segment=1376123&day=2023-9-5&startTime=12:45&endTime=22:30",
...
#     res=requests.get("http://rg.lib.sdyu.edu.cn/api.php/spaces/3787/book",headers=headers)
#
#     # 将新的cookie写入
...
def login_in_1(USERNAME=USERNAME,PASSWORD=PASSWORD):
    ...
    """
    print("■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■")
    res = post(url="http://rg.lib.sdyu.edu.cn/api.php/login",
...
def login_in_2(USERNAME=USERNAME,PASSWORD=PASSWORD):
    ...
    """
    print("■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■")
    res = post(url="http://rg.lib.at.edu.cn/api.php/login",
                   headers={"Referer": "http://www.skalibrary.com/",
                            "User-agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 10_3_1 like Mac OS X) AppleWebKit/603.1.30 (KHTML, like Gecko) Version/10.0 Mobile/14E304 Safari/602.1 Edg/99.0.4844.74"},
                   data={"username": USERNAME, "password": PASSWORD, "from": "mobile"})
    if json.loads(res.content)['status']:
...
def get_area_id():
    ...
    """
    print("■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■")
    res = get(url="http://rg.lib.at.edu.cn/api.php/areas?tree=1",headers={"Referer": "http://www.skalibrary.com/"})
    if json.loads(res.content)['status']:
...
def url_info():
    ...
    for area_id in AREA_ID:
        res = get(
            url=f"http://rg.lib.sdyu.edu.cn/api.php/space_time_buckets?area={area_id}&day={datetime.date.today()}",
            headers={"Referer": "http://www.skalibrary.com/"})
        if json.loads(res.content)['status']:
            ...
            seat_info_url=f"http://rg.lib.sdyu.edu.cn/api.php/spaces_old?area={area_id}&day={day}&endTime={endTime}&segment={id}&startTime={startTime}"
            SEGMENT.append(id)
...
def seat_info(seat_info_url):
    ...
    :param seat_info_url: 传入的网址类似http://rg.lib.sdyu.edu.cn/api.php/spaces_old?area=8&day=2022-07-09&endTime=22:00&segment=1403926&startTime=14:50
    :return:
    """
    print("■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■")
...
def reserve(USERNAME=USERNAME):
    ...
    for SEAT in RESERVE_SEAT_SORTED:
        ...
        # 预约,需要cookie
        res = post(
            url=f"http://rg.lib.sdyu.edu.cn/api.php/spaces/{SEAT[0]}/book",
            headers={"Referer": "http://www.skalibrary.com/",
                     "User-agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 10_3_1 like Mac OS X) AppleWebKit/603.1.30 (KHTML, like Gecko) Version/10.0 Mobile/14E304 Safari/602.1 Edg/99.0.4844.74"},
...
def check_cancel_chance(USERNAME):
    ...
    print("■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■")
    res = get(
        url=f"http://rg.lib.sdyu.edu.cn/api.php/profile/books/",
        headers={
            "Referer": "http://rg.lib.sdyu.edu.cn/user/index/book",
            "User-agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 10_3_1 like Mac OS X) AppleWebKit/603.1.30 (KHTML, like Gecko) Version/10.0 Mobile/14E304 Safari/602.1 Edg/99.0.4844.74",
...
def cancel_reserve(USERNAME=USERNAME):
    ...
    if json.loads(res.content)['data']['list'][0]['statusName'] == '预约开始提醒':
        ...
        # 取消预约
        res = post(
            url=f"http://rg.lib.sdyu.edu.cn/api.php/profile/books/{RESERVED_SEAT[-1]}",
            headers={
                "Referer": "http://rg.lib.sdyu.edu.cn/user/index/book",
                "User-agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 10_3_1 like Mac OS X) AppleWebKit/603.1.30 (KHTML, like Gecko) Version/10.0 Mobile/14E304 Safari/602.1 Edg/99.0.4844.74",
                'Cookie': f'userid={USERNAME};access_token={req.cookies.get("access_token")}'},
...
# 第一次访问 -随便哪个页面http://rg.lib.sdyu.edu.cn/web/seat3?area=8&segment=1315641&day=2122-3-16&startTime=19:21&endTime=22:11
#           -不需要携带cookie
#           -返回   'Set-Cookie':'PHPSESSID=kv8ab5f268pp5mjld6hmtfuus4; path=/; HttpOnly'
# 获取验证码 -http://rg.lib.sdyu.edu.cn/api.php/check
...
# 登录      -http://rg.lib.sdyu.edu.cn/api.php/login
#           -需要携带cookie形式1   'Cookie':'PHPSESSID=kv8ab5f268pp5mjld6hmtfuus4; path=/; HttpOnly'
#           -需要携带cookie形式2   'Cookie':'PHPSESSID=kv8ab5f268pp5mjld6hmtfuus4'
...
# 延长cookie和预约 -"http://rg.lib.sdyu.edu.cn/api.php/spaces/3787/book"
...
# 获取座位信息 -http://rg.lib.sdyu.edu.cn/api.php/check
